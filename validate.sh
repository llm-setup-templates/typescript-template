...
# F1 4 subfacet acceptance -- see .claude/rules/plan-review-deep.md S1
...
for spec in "validate.sh:570" "scaffold.sh:490"; do
  f="${spec%%:*}"; limit="${spec##*:}"
  n=$(wc -l < "$f")
  [[ $n -le $limit ]] || { echo "FAIL: V0a $f has $n lines (limit $limit). Self-ratchet forbidden -- STOP and request R5 refine."; exit 1; }
done
n=$(wc -l < SETUP.md)
...
echo "=== V0e S0 schema guard ==="
# NOTE: section-sign char (\xc2\xa7 UTF-8) stored via printf to keep validate.sh ASCII-only (V23 constraint).
SEC=$(printf '\xc2\xa7')
grep -q "^## ${SEC}0 Phase 0: System Overview" SETUP.md || { echo "FAIL: V0e S0 header missing"; exit 1; }
